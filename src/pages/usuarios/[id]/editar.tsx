...
//   const response = await fetch('/api/admin/profile'); // Endpoint para buscar os perfis
//   if (!response.ok) {
...
const EditUser: React.FC = () => {
  ...
  const [{ data: user }] = useSuspenseQueries( //add  { data: profiles }
    {queries:
    [
    {
      queryKey: ['user', id],
  ...
  const { mutate } = useMutation({
    mutationFn: async ({ email, nome }: FormData) => { //add profile
  ...
  const onSubmit = ({ email, nome }: FormData) => { //add profile

    mutate({ email, nome }) //add profile
  }
